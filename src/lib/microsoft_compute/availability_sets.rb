...
module Azure::ARM::Compute
    ...
    class AvailabilitySet < Azure::ARM::ResourceBase
      ...
      # @return [AvailabilitySetsProperties]
      ...
      def to_h
        self.validate
        hash = {}
        AvailabilitySet.serialize_object(hash,self)
        hash
      end
      ...
      def initialize(parent, init)
        super(parent, init)
        self.type = 'Microsoft.Compute/availabilitySets' if self.type.nil?
        self.api_version = '2015-06-15' if self.api_version.nil?
        if init.is_a? Hash
          AvailabilitySet.deserialize_object self, init.clone
        end 
      end
      ...
      class Configurator < Azure::ARM::ResourceConfigurator
        ...
        # @param properties
        #        AvailabilitySetsProperties
        def properties(props)
          if @parent.properties.nil? and props.is_a? AvailabilitySetsProperties
            @parent.properties = props
            @parent.properties.parent = @parent
            @parent.properties._rsrcpath = 'properties'
          end
          if @parent.properties.nil? and (props.is_a? Hash) and (Azure::ARM::TypeBase.matches_type props, AvailabilitySetsProperties)
            @parent.properties = AvailabilitySetsProperties.new(@parent, props)
            @parent.properties._rsrcpath = 'properties'
          end
          @parent.properties
        end
        ...
        def platform_update_domain_count(props)
          @parent.properties = (AvailabilitySetsProperties.new @parent, nil) if @parent.properties.nil?
          if props.is_a? Fixnum or props.is_a? Azure::ARM::Expression
            @parent.properties.platform_update_domain_count = props
            return
          end
          if @parent.properties.platform_update_domain_count.nil? and props.is_a? Azure::ARM::Expression
            @parent.properties.platform_update_domain_count = props
          end
          @parent.properties.platform_update_domain_count
        end
        ...
        def platform_fault_domain_count(props)
          @parent.properties = (AvailabilitySetsProperties.new @parent, nil) if @parent.properties.nil?
          if props.is_a? Fixnum or props.is_a? Azure::ARM::Expression
            @parent.properties.platform_fault_domain_count = props
            return
          end
          if @parent.properties.platform_fault_domain_count.nil? and props.is_a? Azure::ARM::Expression
            @parent.properties.platform_fault_domain_count = props
          end
          @parent.properties.platform_fault_domain_count
        end
        def initialize(template)
          @template = template
        end
        def create(init=nil,&block)
          @parent = AvailabilitySet.new @template,init
